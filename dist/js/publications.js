current_tab_id = '#publications_nav_item';
...
function publicationDataHandler(store) {
    ...
    function onDone() {
        ...
        function loopArray(arr_elem) {
            ...
            getAllPublications(store, arr_elem['?name'].value, function (out) {
                ...
                var item_html = layoutPublication(template_elem, arr_elem['?name'].value);
    ...
    getOfType(store, 'Publication', storeResults, onDone);
}

// Starting page load with graph data handler
loadGraphData(publicationDataHandler);
